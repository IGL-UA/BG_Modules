...
if(_activated) then {
	if(!isDedicated) then {
			...
			PR(_owner)		= ["east","west","independent","civilian",parseText "playerSide"] select (_logic getVariable "Owner");
			...
			PR(_condition) = _logic getVariable "Condition";
			...
			PR(_func_create_trigger) = {
                ...
                _umova = format ["side player == %1 and (vehicle player) inArea thisTrigger and %2 or side player == %1 and player inArea thisTrigger and %2", _owner, _condition];
